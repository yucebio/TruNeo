...
while(<IN>){
    ...
    next if($startflag == 0);
    $_ =~ s/\s+/\t/g;
    my($pos, $HLA, $peptide, $Identity, $Affinity) = (split "\t", $_)[1, 2, 3, 11, $Affinity_i];
...
while(<INT>){
    ...
    if(exists $position{$Identity}){
        foreach my $i (@{$position{$Identity}}){
            if($length + $pos - 1 >= $i && $pos <= $i){
                $judge = 1;
                last;
            }
        }
    } else{
        $judge = 1 if($length + $pos - 1 >= 15 && $pos <= 15);
    }
    next unless ($judge);
    my $gene = (split "_", $Identity)[0];
    my $key = join ",", $pos, $HLA, $gene, $length;
    ...
    my $freq = ($widetype-$Affinity)/$Affinity;
    next if($widetype < 500 || $freq < 0.5);

    my $out = join "\t", $pos, $HLA, $peptide, $Identity, $Affinity, $type, $widetype, $widetypepep;
    print "$out\n";
}
#===============================================================================
#
